...
import {private_createTypography, Typography} from '@mui/material';
...
const TableDetail = ({route, navigation}) => {
  ...
  useEffect(() => {
    getRequest(
      'orders/getOrdersByTableId?tableId=' + tableId,
      responseData => {
        setIsLoading(true);
        ...
        responseData.map(item => {
          ...
          newProductList.push(productBody);
          const orderBody = {
            id: item.product.id,
            name: item.product.name,
            price: item.product.price,
  ...
  const getProductByCategory = async e => {
    ...
    getRequest(
      'product/getProductsByCategory?categoryId=' + e.id,
      responseData => {
        if (responseData.status === 200) {
          const newData = responseData.result.map(item => {
            const newQuantity = selectedProducts.filter(
              row => row.id === item.id && row.quantity > 0,
            );
          ...
          <View style={styles.modalView}>
            <View style={styles.menu}>
              <FlatList
                ...
                renderItem={({item}) => (
                  <TouchableOpacity
                    onPress={() => getProductByCategory(item)}
                    disabled={!isEnabled}>
                    <Text
                      style={[
                        styles.menuItem,
